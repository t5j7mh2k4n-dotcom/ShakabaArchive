...
/// <summary>تسجيل ودخول عبر Firebase Identity Toolkit REST API.</summary>
public class FirebaseAuthService(HttpClient http, IOptions<FirebaseOptions> options)
{
    ...
    public async Task<(bool Ok, string Error, string? ErrorCode, string? LocalId)> SignUpAsync(
        string email,
        string password,
        CancellationToken ct = default)
    {
        ...
        if (res.IsSuccessStatusCode && body?.LocalId is { Length: > 0 })
            return (true, "", null, body.LocalId);

        var code = body?.Error?.Message;
        return (false, MapError(code), code, null);
    }

    public async Task<(bool Ok, string Error, string? LocalId)> SignInAsync(
        string email,
        string password,
        CancellationToken ct = default)
    {
        ...
        var body = await res.Content.ReadFromJsonAsync<FirebaseAuthResponse>(cancellationToken: ct);
        if (res.IsSuccessStatusCode && body?.LocalId is { Length: > 0 })
            return (true, "", body.LocalId);

        return (false, MapError(body?.Error?.Message), null);
    }
